<?php
/**
 * Elementor widget: Product Category Filter Bar (Category + Price + Sort).
 *
 * The category-archive counterpart to the Brand Filter Bar. Output is delegated
 * to JPWBC_Filter::render_category_filter_bar() so the widget and the
 * [jpwbc_category_filter] shortcode produce identical markup.
 *
...
/**
 * Product Category Filter Bar Elementor widget.
 *
 * @since 1.20.0
 */
class JPWBC_Elementor_Category_Filter extends \Elementor\Widget_Base {
	...
	public function get_title(): string {
		return __( 'Product Category Filter Bar', 'jezpress-woo-brand-categories' );
	}
	...
	public function get_keywords(): array {
		return array( 'category', 'product category', 'filter', 'price', 'sort', 'shop', 'woocommerce', 'jezpress' );
	}
	...
	protected function register_controls(): void {
		$this->start_controls_section(
			'jpwbc_section',
			array( 'label' => __( 'Product Category Filter Bar', 'jezpress-woo-brand-categories' ) )
		);

		$this->add_control(
			'jpwbc_category',
			array(
				'label'       => __( 'Category slug (optional)', 'jezpress-woo-brand-categories' ),
				'type'        => \Elementor\Controls_Manager::TEXT,
				'default'     => '',
				'description' => __( 'Leave blank to use the category archive being viewed. Set a slug to force a specific category.', 'jezpress-woo-brand-categories' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'description'  => __( 'Lists every product category, grouped under its parent. Parents with subcategories expand in place.', 'jezpress-woo-brand-categories' ),
		...
		$this->add_control(
			...
			array(
				...
				'description'  => __( 'Colour / Style / … facets built from the products in this category (needs the attribute index — rebuild it under JezPress > Brand Categories > Cache).', 'jezpress-woo-brand-categories' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => 'yes',
				'return_value' => 'yes',
			)
	...
	protected function render(): void {
		...
		$category = isset( $settings['jpwbc_category'] ) ? sanitize_title( (string) $settings['jpwbc_category'] ) : '';

		// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- render_category_filter_bar() returns template output that escapes every sink; a one-line ignore does not cover the multi-line argument.
		echo $filter->render_category_filter_bar(
			array(
				'category'         => $category,
